...
void CLI::runCommand(String input) {
    ...
    // screen mode <menu/packetmonitor/buttontest/loading>
        ...
        if (eqlsCMD(2, CLI_MODE_BUTTONTEST)) displayUI.setMode(DISPLAY_MODE::BUTTON_TEST);
        else if (eqlsCMD(2, CLI_MODE_PACKETMONITOR)) displayUI.setMode(DISPLAY_MODE::PACKETMONITOR);
        else if (eqlsCMD(2, CLI_MODE_LOADINGSCREEN)) displayUI.setMode(DISPLAY_MODE::LOADSCAN);
        else if (eqlsCMD(2, CLI_MODE_MENU)) displayUI.setMode(DISPLAY_MODE::MENU);
        else parameterError(list->get(2));
